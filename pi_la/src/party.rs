...
pub struct Party {
    ...
    pub dealer_proof: Option<(Vec<Vec<u8>>, Polynomial)>,
    ...
    pub fn verify_shares(
        ...
        match &self.dealer_proof {
            Some((cvals, z)) => match &self.shares {
                Some(shares) => {
                    let flat_vec: Vec<u8> = cvals.clone().into_iter().flatten().collect();
                    ...
                    self.validated_shares = shares
                        ...
                        .map_init(
                            ...
                            |(l_hasher, l_buf), (i, fi)| {
                                ...
                                l_hasher.update((z.evaluate(i + 1) - d * fi).as_bytes());
